...
So here we begin opening it up, the mainboard is encased within a metal box  which then sites below the CD player in the lower part of the casing.
...
The blue sides of the capacitors show where the negative terminal is, the red arrow shows where it should be, so as we can see both of these are installed incorrectly with the polarity not as it should be. Now focus on the top of the Capacitors themselves, the metal roof of these should be flat and clean, instead we can see that they bulge and there are visible signs of leakage.

[<img src="{{ site.baseurl }}/images/cd32-recap/7.jpg"
            style="width: 800px;"/>]({{ site.baseurl }}/images/cd32-recap/7.jpg)                    

When we remove them we can see that even above that even the guides on the mainboard were incorrect. The guides clearely show that which terminal should be used for positive and which for negative, so this is why so many of these boards share the same issue. We're going to ignore these guides and install correctly, the "wrong" way round!
...
And then put it back together and power it on:
